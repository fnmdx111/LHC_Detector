...
{
  expHall_x = expHall_y = expHall_z = 400.0*mm;
}
...
//The glass box 
//
G4Box* glassBox = new G4Box("glassBox", 5.*mm,200.*mm,200.*mm);
...
//the inter layer
//
G4Box* interLayer = new G4Box("ironLayer", 20.*mm, 200.*mm, 200.*mm);
...
//PMTSD construction
//
G4Box* sdBox = new G4Box("SD_Box", 5.*mm, 200.*mm, 0*mm);
...
//The Glass Fiber
//
G4Tubs* fiber_core = new G4Tubs("glassFiber_core",0,0.6*mm,210.*mm,0,360);
G4Tubs* fiber_clad = new G4Tubs("glassFiber_clad",0,1.*mm, 210.*mm,0,360);
...
//
for (G4int copy = 0; copy < 9; copy ++)
{
	G4double loc_z = -200*mm + 40*mm * (copy+1);
...
for (G4int copy = 0; copy < 8; copy ++)
{
	G4double loc_z = -180*mm + 40*mm * (copy+1);
	G4ThreeVector loc_fiber(-1*mm,0,loc_z*mm);
...
G4double loc_z = -250*mm + 50*mm * (copy);
	G4double loc_i = loc_z + 25*mm;
	//place of glass plate
	G4ThreeVector loc_fiber(loc_z*mm, 0, 0);
	//place of sd 
	G4ThreeVector loc_sd1(loc_z*mm, 210*mm, 0);
	G4ThreeVector loc_sd2(loc_z*mm, -210*mm,0);
